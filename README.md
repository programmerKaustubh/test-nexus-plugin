...
5. **Firebase CLI** installed — run `npm install -g firebase-tools` if you haven't already
...
Download the extension package from the TestNexus website:

1. Go to [twocan.us/extension](https://twocan.us/extension) and download the latest extension package
2. Unzip the downloaded file to a convenient location

Open a terminal, navigate to the unzipped folder, install dependencies, and install the extension:
...
> **Important:** The unzipped folder includes a `firebase.json` file. If you already have a `firebase.json` in your working directory, run the install command from a separate directory to avoid conflicts.
...
If you have your token ready and want to install without being prompted for parameters, run from inside the unzipped extension folder:
...
Before adding a new token, you need to know which tokens are already configured. You can view them in the Google Cloud Console under **Secret Manager**, or run:
...
1. **See current tokens** — use the `gcloud` command above or check Secret Manager in Google Cloud Console
...
- **Issues & Feedback:** support@twocan.us

## License

Copyright (c) 2026 Twocan Software LLC. All Rights Reserved. See [LICENSE](LICENSE) for details.
